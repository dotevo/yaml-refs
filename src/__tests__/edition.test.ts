...
describe('include file test (2st res)', () => {
  ...
  test('edit parent should modify children', () => {
    ...
    i.set('color', 'yellow');
    expect(doc.getIn(['child']).toJSON()).toEqual(JSON.parse('{"animal": "cat", "color": "yellow", "uniqueParent": 1, "uniqueChild": 2}'));
  });
  ...
  test('edit child should modify parent if value comes from parent', () => {
    ...
    i.set('color', 'yellow');
    expect(doc.getIn(['parent']).toJSON()).toEqual(JSON.parse('{"animal": "dog", "color": "yellow", "uniqueParent": 1}'));
  ...
  test('edit child should not modify parent if it is own value', () => {
    ...
    const i = doc.getIn(['child']);
    i.set('animal', 'bird');
    expect(doc.getIn(['parent']).toJSON()).toEqual(JSON.parse('{"animal": "dog", "color": "black", "uniqueParent": 1}'));
    expect(doc.getIn(['child']).toJSON()).toEqual(JSON.parse('{"animal": "bird", "color": "black", "uniqueParent": 1, "uniqueChild": 2}'));
  });
  ...
  test('edit unique value in child', () => {
    ...
    i.set('uniqueChild', 5);
    expect(doc.getIn(['parent']).toJSON()).toEqual(JSON.parse('{"animal": "dog", "color": "black", "uniqueParent": 1}'));
    expect(doc.getIn(['child']).toJSON()).toEqual(JSON.parse('{"animal": "cat", "color": "black", "uniqueChild": 5, "uniqueParent": 1}'));
  });
  ...
  test('edit child should not modify parent if wants to create new value', () => {
    ...
    expect(doc.getIn(['parent']).toJSON()).toEqual(JSON.parse('{"animal": "dog", "color": "black", "uniqueParent": 1}'));
    expect(doc.getIn(['child']).toJSON()).toEqual(JSON.parse('{"animal": "cat", "color": "red", "uniqueParent": 1, "uniqueChild": 2}'));
